...
This algorithm is the exact same as that in <span style="color:blue;"><tt>bSAT_HOPPET.R</tt></span>. The gluon density is evolved directly by calling HOPPET from within this code.
...
# Export the initial gluon density parameters to a file called "parameters.txt"
...
# Meson-dependent constants (for rho)
...
A_L = function (variables, x_Bj, Q2, t) {
  ...
  # For a Windows machine, change system() to shell()
  
  xg = as.numeric(system("./tabulation_example", intern = TRUE))
...
bSAT_Q2 = c(2, 20, 40, 100)

# Calculate the b-SAT predictions
# Use lapply() (or mapply()) to calculate multiple data points more quickly

bSAT_vs_Q2_rho = matrix(unlist(lapply(bSAT_Q2, bSAT, 75)), ncol = length(bSAT_Q2), byrow = FALSE)
